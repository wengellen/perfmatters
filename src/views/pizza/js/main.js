...
var ticking = false;  // flag to check if requestanimationframe has been requested
var scrollTop = 0;    // variable to store the amount of space has been scrolled

// Utility function to convert DOM collection into Array
...
// This function is used to calculate and update pizza image element's position
function updatePosition() {
  ...
  divs.forEach(function(elem, index, arr){
    phase = Math.sin(scrollTop/1250 + (index % limit));
    left = elem.basicLeft + 100 * phase + 'px';
    leftString = "translate3d(" + left + "," + 0 + ", " + 0 + ")";
    elem.style.webkitTransform = leftString;
    elem.style.transform = leftString;
  });
...
function onScroll(evt) {
  if(!ticking) {
    // get scrollTop value based on different browsers
